...
public class IndividuoRecorrido extends Individuo<Integer>{	
...
private final static int[][] _DIST = {
	...
	public IndividuoRecorrido() {
		this.isMin = true;
		this.tamTotal = 27;
		this.cromosoma = new Integer[27];
		Boolean existe[] = new Boolean[27];
		Arrays.fill(existe, false);
		int i = 0;
		while(i < 27) {
			int ciudad = this.rand.nextInt(0, 27);
			if(!existe[ciudad]) {
	...
	public int getFitness() {
		...
		int distTotal = 0;
		int primera = this.cromosoma[0];
		for(int i = 0; i < 26; i++) {
			if(this.cromosoma[i] > this.cromosoma[i+1]) 
				distTotal += _DIST[this.cromosoma[i]][this.cromosoma[i+1]];
			else
				distTotal += _DIST[this.cromosoma[i+1]][this.cromosoma[i]];
		}
		if(this.cromosoma[26] > primera) 
			distTotal += _DIST[this.cromosoma[26]][primera];
		else
			distTotal += _DIST[primera][this.cromosoma[26]];
		
		return distTotal;
	}

	@Override
	public int getFenotipo(int n) {
		// TODO Auto-generated method stub
