...
public class AnalisisView extends JDialog{
	...
	private static int tamPob, maxGen, tipSel, tipCruc, tipMut;
	...
	public AnalisisView(int tamPob, int maxGen, double probCruc, double probMut,int tipSel, int tipCruc, int tipMut, double elit, 
		...
		this.panel.add(this.plot, BorderLayout.CENTER);
	...
	private static void run() {
		...
		if(minVal < maxVal) {
			while(!fin) {
				if(TipoAnalisis.getSelectedIndex() == 0) 
					al = new AlgoritmoGenetico(tamPob, minVal, probMut, tipSel, tipCruc, tipMut, elit);
				else if(TipoAnalisis.getSelectedIndex() == 1) 
					al = new AlgoritmoGenetico(tamPob, probCruc, minVal, tipSel, tipCruc, tipMut, elit);
				else if(TipoAnalisis.getSelectedIndex() == 2) 
					al = new AlgoritmoGenetico((int) minVal, probCruc, probMut, tipSel, tipCruc, tipMut, elit);
				else if(TipoAnalisis.getSelectedIndex() == 3) 
					al = new AlgoritmoGenetico(tamPob, probCruc, probMut, tipSel, tipCruc, tipMut, minVal);
